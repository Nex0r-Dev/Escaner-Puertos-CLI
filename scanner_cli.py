...
def get_mac_address(ip: str) -> str:
    try:
        output = subprocess.check_output(["arp", "-n", ip], stderr=subprocess.DEVNULL).decode("utf-8")
        ...
        try:
            output = subprocess.check_output(["arp", ip], stderr=subprocess.DEVNULL).decode("utf-8")
            for line in output.split("\n"):
                if ip in line:
                    parts = line.split()
                    for part in parts:
...
def detect_os(ip: str, timeout: float = 0.5) -> str:
    os_hint = "Unknown"
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            try:
                ...
                try:
                    ...
                    pass
                if banner:
                    banner_str = banner.decode(errors="ignore").lower()
                    if "openssh" in banner_str:
                        if "ubuntu" in banner_str or "debian" in banner_str:
                            os_hint = "Linux (Ubuntu/Debian)"
                        elif "alpine" in banner_str:
                            os_hint = "Linux (Alpine)"
                        elif "centos" in banner_str or "rhel" in banner_str or "fedora" in banner_str:
                            os_hint = "Linux (RHEL/CentOS)"
                        else:
                            os_hint = "Linux"
                    elif "libssh" in banner_str or "paramiko" in banner_str:
                        os_hint = "Linux/Unix"
                    else:
                        os_hint = "Unix-like"
                    return os_hint
...
def discover_network(subnet: Optional[str], timeout: float, workers: int) -> List[Tuple[str, str, str, str]]:
            ...
            try:
                if future.result():
                    try:
                        ...
                        name = "-"
                    os_detected = detect_os(ip, timeout)
                    mac_address = get_mac_address(ip)
                    found.append((ip, name, os_detected, mac_address))
            except Exception:
                pass
...
def run():
    ...
    if args.discover:
        ...
        if devices:
            ...
            print(f"{'#':>2}  {'IP':>15}  {'Nombre':>20}  {'MAC':>18}  {'OS':>15}")
            print(f"{'-'*2}  {'-'*15}  {'-'*20}  {'-'*18}  {'-'*15}")
            for index, (ip, name, os_detected, mac) in enumerate(devices, start=1):
                name_display = name if name != "-" else "Sin DNS"
                mac_display = mac if mac != "Unknown" else "Unknown"
                print(f"{index:2}  {ip:>15}  {name_display:>20}  {mac_display:>18}  {os_detected:>15}")
            ...
            if choice.isdigit():
                ...
                if 0 <= index < len(devices):
                    selected_ip, selected_name, selected_os, selected_mac = devices[index]
                else:
                    print(f"{COLOR_YELLOW}Número inválido. Saliendo.{COLOR_RESET}")
                    return
            else:
                for ip, name, os_detected, mac in devices:
                    if ip == choice:
                        selected_ip = ip
                        selected_name = name
                        selected_os = os_detected
                        selected_mac = mac
                        break
                if not selected_ip:
            ...
            device_name = selected_name if selected_name != "-" else "Sin nombre DNS"
            mac_display = selected_mac if selected_mac != "Unknown" else "No disponible"
            print(f"\n{COLOR_BLUE}{'='*70}{COLOR_RESET}")
            print(f"{COLOR_CYAN}Dispositivo seleccionado:{COLOR_RESET}")
            print(f"  {COLOR_GREEN}Nombre:{COLOR_RESET} {device_name}")
            print(f"  {COLOR_GREEN}IP:{COLOR_RESET} {selected_ip}")
            print(f"  {COLOR_GREEN}MAC Address:{COLOR_RESET} {mac_display}")
            print(f"  {COLOR_GREEN}Sistema Operativo:{COLOR_RESET} {selected_os}")
            print(f"{COLOR_BLUE}{'='*70}{COLOR_RESET}")
            print(f"\n{COLOR_BLUE}>>>{COLOR_RESET} Escaneando con {len(ports)} puertos y {args.workers} hilos...")
            start = time.time()
            try:
            ...
            format_results(device_name, selected_ip, results, elapsed)

            if args.file:
                try:
                    save_results(device_name, selected_ip, results, args.file)
